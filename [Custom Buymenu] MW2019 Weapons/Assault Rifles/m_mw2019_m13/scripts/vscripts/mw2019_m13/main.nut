mcharlie_MDL <- "models/weapons/v_ar_m13.mdl"
mcharlie_sequence <- 6

function mcharlie_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("mcharlie_owned"))
		{
			draw_scope.mcharlie_FD <- false
			draw_scope.mcharlie_owned <- false
		}
		
		draw_scope.mcharlie_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, mcharlie_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.mcharlie_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.mcharlie_owned = false;
		if (draw_scope.mcharlie_owned == false) draw_scope.mcharlie_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != mcharlie_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("mcharlie_FD") )
		{
			draw_scope.mcharlie_FD <- false
			draw_scope.mcharlie_owned <- false
		}
		
		if (draw_scope.mcharlie_FD == false)
		{
			vm.__KeyValueFromInt("sequence", mcharlie_sequence)
			draw_scope.mcharlie_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mcharlie_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mcharlie_deployCheck():0:-1", 0, null, null)
