mike14_MDL <- "models/weapons/v_sn_ebr14.mdl"
mike14_sequence <- 8

function mike14_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("mike14_owned"))
		{
			draw_scope.mike14_FD <- false
			draw_scope.mike14_owned <- false
		}
		
		draw_scope.mike14_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, mike14_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.mike14_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.mike14_owned = false;
		if (draw_scope.mike14_owned == false) draw_scope.mike14_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != mike14_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("mike14_FD") )
		{
			draw_scope.mike14_FD <- false
			draw_scope.mike14_owned <- false
		}
		
		if (draw_scope.mike14_FD == false)
		{
			vm.__KeyValueFromInt("sequence", mike14_sequence)
			draw_scope.mike14_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike14_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike14_deployCheck():0:-1", 0, null, null)
