sksierra_MDL <- "models/weapons/v_sn_sks.mdl"
sksierra_sequence <- 8

function sksierra_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("sksierra_owned"))
		{
			draw_scope.sksierra_FD <- false
			draw_scope.sksierra_owned <- false
		}
		
		draw_scope.sksierra_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, sksierra_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.sksierra_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.sksierra_owned = false;
		if (draw_scope.sksierra_owned == false) draw_scope.sksierra_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != sksierra_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("sksierra_FD") )
		{
			draw_scope.sksierra_FD <- false
			draw_scope.sksierra_owned <- false
		}
		
		if (draw_scope.sksierra_FD == false)
		{
			vm.__KeyValueFromInt("sequence", sksierra_sequence)
			draw_scope.sksierra_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:sksierra_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:sksierra_deployCheck():0:-1", 0, null, null)
