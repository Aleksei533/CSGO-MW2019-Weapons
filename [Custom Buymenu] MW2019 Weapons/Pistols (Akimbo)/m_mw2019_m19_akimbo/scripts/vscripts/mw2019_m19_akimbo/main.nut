papa320_dual_MDL <- "models/weapons/v_pi_m19_akimbo.mdl"
papa320_dual_sequence <- 8

function papa320_dual_GetAmmoState(vm)
{
...
function papa320_dual_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("papa320_dual_owned"))
		{
			draw_scope.papa320_dual_FD <- false
			draw_scope.papa320_dual_owned <- false
		}
		
		draw_scope.papa320_dual_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, papa320_dual_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_elite" && wpnInst.GetOwner() == ply)
			{
				draw_scope.papa320_dual_owned = true;
				break;
			}
		}
		
		if (ply.GetHealth() < 1) draw_scope.papa320_dual_owned = false;
		if (draw_scope.papa320_dual_owned == false) draw_scope.papa320_dual_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != papa320_dual_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("papa320_dual_FD") )
		{
			draw_scope.papa320_dual_FD <- false
			draw_scope.papa320_dual_owned <- false
		}
		
		if (draw_scope.papa320_dual_FD == false)
		{
			if (papa320_dual_GetAmmoState(vm) != "empty") vm.__KeyValueFromInt("sequence", papa320_dual_sequence)
			draw_scope.papa320_dual_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:papa320_dual_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:papa320_dual_deployCheck():0:-1", 0, null, null)
