mike9_dual_MDL <- "models/weapons/v_pi_renetti_akimbo.mdl"
mike9_dual_sequence <- 6

function mike9_dual_GetAmmoState(vm)
{
...
function mike9_dual_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("mike9_dual_owned"))
		{
			draw_scope.mike9_dual_FD <- false
			draw_scope.mike9_dual_owned <- false
		}
		
		draw_scope.mike9_dual_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, mike9_dual_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_elite" && wpnInst.GetOwner() == ply)
			{
				draw_scope.mike9_dual_owned = true;
				break;
			}
		}
		
		if (ply.GetHealth() < 1) draw_scope.mike9_dual_owned = false;
		if (draw_scope.mike9_dual_owned == false) draw_scope.mike9_dual_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != mike9_dual_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("mike9_dual_FD") )
		{
			draw_scope.mike9_dual_FD <- false
			draw_scope.mike9_dual_owned <- false
		}
		
		if (draw_scope.mike9_dual_FD == false)
		{
			if (mike9_dual_GetAmmoState(vm) != "empty") vm.__KeyValueFromInt("sequence", mike9_dual_sequence)
			draw_scope.mike9_dual_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike9_dual_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike9_dual_deployCheck():0:-1", 0, null, null)
