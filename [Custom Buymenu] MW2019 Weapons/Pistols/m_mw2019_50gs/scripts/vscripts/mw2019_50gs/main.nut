decho_MDL <- "models/weapons/v_pi_50gs.mdl"
decho_sequence <- 6
...
function decho_deployCheck()
{
	...
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("decho_owned"))
		{
			draw_scope.decho_FD <- false
			draw_scope.decho_owned <- false
		}
		
		draw_scope.decho_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, decho_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.decho_owned = true;
				break;
			}
		}
		
		if (ply.GetHealth() < 1) draw_scope.decho_owned = false;
		if (draw_scope.decho_owned == false) draw_scope.decho_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != decho_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("decho_FD") )
		{
			draw_scope.decho_FD <- false
			draw_scope.decho_owned <- false
		}
		
		if (draw_scope.decho_FD == false)
		{
			if (decho_GetAmmoState(vm) != "empty") vm.__KeyValueFromInt("sequence", decho_sequence)
			draw_scope.decho_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:decho_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:decho_deployCheck():0:-1", 0, null, null)
