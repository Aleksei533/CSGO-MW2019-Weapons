mike9_MDL <- "models/weapons/v_pi_renetti.mdl"
mike9_sequence <- 6

function mike9_GetAmmoState(vm)
{
...
function mike9_deployCheck()
{
	...
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("mike9_owned"))
		{
			draw_scope.mike9_FD <- false
			draw_scope.mike9_owned <- false
		}
		
		draw_scope.mike9_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, mike9_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.mike9_owned = true;
				break;
			}
		}
		
		if (ply.GetHealth() < 1) draw_scope.mike9_owned = false;
		if (draw_scope.mike9_owned == false) draw_scope.mike9_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != mike9_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("mike9_FD") )
		{
			draw_scope.mike9_FD <- false
			draw_scope.mike9_owned <- false
		}
		
		if (draw_scope.mike9_FD == false)
		{
			if (mike9_GetAmmoState(vm) != "empty") vm.__KeyValueFromInt("sequence", mike9_sequence)
			draw_scope.mike9_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike9_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike9_deployCheck():0:-1", 0, null, null)
