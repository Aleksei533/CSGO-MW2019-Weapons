...
projectileModel <- "models/weapons/w_la_jokr_rocket.mdl"
...
function fireRocket()
{
	local glitchedRocketUser = Entities.FindByName(null, "rocketman")
	if(glitchedRocketUser) 
		glitchedRocketUser.__KeyValueFromString("targetname", "")
	...
	{		
		...
		if(hit)
		{
			...
			// Effects
			self.StopSound( "Weapon_JOKR.Travel" )
...
function checkRPGFire()
{
	...
	EntFire("JOKR_rocket", "RunScriptCode", "think()")
	...
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if( current_anim == "fire" )
		{
			...
			if( !scope.rawin("JOKR_lastFire") )
				scope.JOKR_lastFire <- 0
			
			// If it has been long enough, fire.
			if( scope.JOKR_lastFire < Time()-refireDelay )
			{
				scope.JOKR_lastFire = Time()
...
EntFireByHandle(lmm, "AddOutput", "targetname JOKR_lmm", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "classname move_rope", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "MeasureType 1", 0, null, null)
EntFireByHandle(lmm, "SetMeasureReference", "JOKR_lmm", 0, null, null)
EntFireByHandle(lmm, "SetTargetReference", "JOKR_lmm", 0, null, null)
EntFireByHandle(lmm, "SetMeasureTarget", "rocketman", 0, null, null)
EntFireByHandle(lmm, "SetTarget", "JOKR_lmm", 0, null, null)
