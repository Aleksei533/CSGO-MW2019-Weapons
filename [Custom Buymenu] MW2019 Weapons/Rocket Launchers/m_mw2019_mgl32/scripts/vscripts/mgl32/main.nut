...
{
	local ply = null
	foreach(plyClass in ["player", "cs_bot"])
	while (ply = Entities.FindByClassname(ply, plyClass))
	{
		...
		draw_scope.mike32_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByClassname(wpnInst, "weapon_*"))
		{
			if (wpnInst.GetOwner() == ply && wpnInst.GetModelName() == mike32_MDL)
			{
...
mike32_deploy_timer <- Entities.CreateByClassname("logic_timer")
EntFireByHandle(mike32_deploy_timer, "AddOutput", "Refiretime 0.1", 0, null, null)
EntFireByHandle(mike32_deploy_timer, "AddOutput", "classname move_rope", 0, null, null)
EntFireByHandle(mike32_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mike32_deployCheck():0:-1", 0, null, null)
EntFireByHandle(mike32_deploy_timer, "Enable", "", 0.1, null, null)
...
projectileModel <- "models/weapons/w_la_mgl32_charge.mdl"
...
function VM_RPGGetAnim(vm)
{
	...
	else if (org_dist.z > 1.8 && org_dist.z < 2.2) tracked_anim = "draw";
	else if (org_dist.z > 3.8 && org_dist.z < 4.2) tracked_anim = "draw_first";
...
function fireRocket()
{
	...
	{		
		...
		if(hit)
		{
			...
			// Effects
			self.StopSound( "Weapon_RPG.Travel" )
...
function checkRPGFire()
{
	...
	EntFire("rpg_rocket", "RunScriptCode", "think()")
	...
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if( current_anim == "fire" )
		{
			...
			if( !scope.rawin("rpg_lastFire") )
				scope.rpg_lastFire <- 0
			
			// If it has been long enough, fire.
			if( scope.rpg_lastFire < Time()-refireDelay )
			{
				scope.rpg_lastFire = Time()
...
EntFireByHandle(lmm, "AddOutput", "targetname rpg_lmm", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "classname move_rope", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "MeasureType 1", 0, null, null)
EntFireByHandle(lmm, "SetMeasureReference", "rpg_lmm", 0, null, null)
EntFireByHandle(lmm, "SetTargetReference", "rpg_lmm", 0, null, null)
EntFireByHandle(lmm, "SetMeasureTarget", "rocketman", 0, null, null)
EntFireByHandle(lmm, "SetTarget", "rpg_lmm", 0, null, null)
