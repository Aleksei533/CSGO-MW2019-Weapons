...
projectileModel <- "models/weapons/w_la_rpg_rocket.mdl"
...
function fireRocket()
{
	...
	rocket.__KeyValueFromString("targetname", "MW_RPG_rocket")
...
function checkRPGFire()
{
	...
	EntFire("MW_RPG_rocket", "RunScriptCode", "think()")
	...
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if( current_anim == "fire" )
		{
			...
			if( !scope.rawin("MW_RPG_lastFire") )
				scope.MW_RPG_lastFire <- 0
			
			// If it has been long enough, fire.
			if( scope.MW_RPG_lastFire < Time()-refireDelay )
			{
				scope.MW_RPG_lastFire = Time()
...
EntFireByHandle(lmm, "AddOutput", "targetname MW_RPG_lmm", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "classname move_rope", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "MeasureType 1", 0, null, null)
EntFireByHandle(lmm, "SetMeasureReference", "MW_RPG_lmm", 0, null, null)
EntFireByHandle(lmm, "SetTargetReference", "MW_RPG_lmm", 0, null, null)
EntFireByHandle(lmm, "SetMeasureTarget", "rocketman", 0, null, null)
EntFireByHandle(lmm, "SetTarget", "MW_RPG_lmm", 0, null, null)
