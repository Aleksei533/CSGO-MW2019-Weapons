...
projectileModel <- "models/weapons/w_la_strela-p_rocket.mdl"
...
function fireRocket()
{
	...
	{		
		...
		if(hit)
		{
			...
			// Effects
			self.StopSound( "Weapon_Strela.Travel" )
...
function checkRPGFire()
{
	...
	EntFire("STRELA_rocket", "RunScriptCode", "think()")
	...
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if( current_anim == "fire" )
		{
			...
			if( !scope.rawin("STRELA_lastFire") )
				scope.STRELA_lastFire <- 0
			
			// If it has been long enough, fire.
			if( scope.STRELA_lastFire < Time()-refireDelay )
			{
				scope.STRELA_lastFire = Time()
...
EntFireByHandle(lmm, "AddOutput", "targetname STRELA_lmm", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "classname move_rope", 0, null, null)
EntFireByHandle(lmm, "AddOutput", "MeasureType 1", 0, null, null)
EntFireByHandle(lmm, "SetMeasureReference", "STRELA_lmm", 0, null, null)
EntFireByHandle(lmm, "SetTargetReference", "STRELA_lmm", 0, null, null)
EntFireByHandle(lmm, "SetMeasureTarget", "rocketman", 0, null, null)
EntFireByHandle(lmm, "SetTarget", "STRELA_lmm", 0, null, null)
