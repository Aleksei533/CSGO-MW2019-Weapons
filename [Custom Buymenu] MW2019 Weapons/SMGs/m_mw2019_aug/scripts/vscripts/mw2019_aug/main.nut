augolf_MDL <- "models/weapons/v_sm_aug.mdl"
augolf_sequence <- 5

function augolf_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("augolf_owned"))
		{
			draw_scope.augolf_FD <- false
			draw_scope.augolf_owned <- false
		}
		
		draw_scope.augolf_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, augolf_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.augolf_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.augolf_owned = false;
		if (draw_scope.augolf_owned == false) draw_scope.augolf_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != augolf_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("augolf_FD") )
		{
			draw_scope.augolf_FD <- false
			draw_scope.augolf_owned <- false
		}
		
		if (draw_scope.augolf_FD == false)
		{
			vm.__KeyValueFromInt("sequence", augolf_sequence)
			draw_scope.augolf_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:augolf_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:augolf_deployCheck():0:-1", 0, null, null)
