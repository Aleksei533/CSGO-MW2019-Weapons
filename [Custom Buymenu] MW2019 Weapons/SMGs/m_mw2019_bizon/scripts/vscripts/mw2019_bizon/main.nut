beta_MDL <- "models/weapons/v_sm_bizon.mdl"
beta_sequence <- 5

function beta_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("beta_owned"))
		{
			draw_scope.beta_FD <- false
			draw_scope.beta_owned <- false
		}
		
		draw_scope.beta_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, beta_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.beta_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.beta_owned = false;
		if (draw_scope.beta_owned == false) draw_scope.beta_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != beta_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("beta_FD") )
		{
			draw_scope.beta_FD <- false
			draw_scope.beta_owned <- false
		}
		
		if (draw_scope.beta_FD == false)
		{
			vm.__KeyValueFromInt("sequence", beta_sequence)
			draw_scope.beta_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:beta_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:beta_deployCheck():0:-1", 0, null, null)
