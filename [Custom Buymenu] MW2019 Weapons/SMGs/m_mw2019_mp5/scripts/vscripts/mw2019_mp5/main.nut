mpapa5_MDL <- "models/weapons/v_sm_mp5.mdl"
mpapa5_sequence <- 5

function mpapa5_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("mpapa5_owned"))
		{
			draw_scope.mpapa5_FD <- false
			draw_scope.mpapa5_owned <- false
		}
		
		draw_scope.mpapa5_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, mpapa5_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.mpapa5_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.mpapa5_owned = false;
		if (draw_scope.mpapa5_owned == false) draw_scope.mpapa5_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != mpapa5_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("mpapa5_FD") )
		{
			draw_scope.mpapa5_FD <- false
			draw_scope.mpapa5_owned <- false
		}
		
		if (draw_scope.mpapa5_FD == false)
		{
			vm.__KeyValueFromInt("sequence", mpapa5_sequence)
			draw_scope.mpapa5_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mpapa5_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:mpapa5_deployCheck():0:-1", 0, null, null)
