aalpha12_MDL <- "models/weapons/v_sh_jak12.mdl"
aalpha12_sequence <- 6

function aalpha12_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("aalpha12_owned"))
		{
			draw_scope.aalpha12_FD <- false
			draw_scope.aalpha12_owned <- false
		}
		
		draw_scope.aalpha12_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, aalpha12_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.aalpha12_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.aalpha12_owned = false;
		if (draw_scope.aalpha12_owned == false) draw_scope.aalpha12_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != aalpha12_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("aalpha12_FD") )
		{
			draw_scope.aalpha12_FD <- false
			draw_scope.aalpha12_owned <- false
		}
		
		if (draw_scope.aalpha12_FD == false)
		{
			vm.__KeyValueFromInt("sequence", aalpha12_sequence)
			draw_scope.aalpha12_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:aalpha12_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:aalpha12_deployCheck():0:-1", 0, null, null)
