hdromeo_MDL <- "models/weapons/v_sn_hdr.mdl"
hdromeo_sequence <- 7

function hdromeo_deployCheck()
{
	// Find all viewmodels
	local vm = null
	while( vm = Entities.FindByClassname(vm, "predicted_viewmodel") )
	{
		...
		if(!draw_scope.rawin("hdromeo_owned"))
		{
			draw_scope.hdromeo_FD <- false
			draw_scope.hdromeo_owned <- false
		}
		
		draw_scope.hdromeo_owned = false;
		local wpnInst = null;
		while (wpnInst = Entities.FindByModel(wpnInst, hdromeo_MDL))
		{
			if (wpnInst.GetClassname() == "weapon_deagle" && wpnInst.GetOwner() == ply)
			{
				draw_scope.hdromeo_owned = true;
				break;
			}
		}
		if (ply.GetHealth() < 1) draw_scope.hdromeo_owned = false;
		if (draw_scope.hdromeo_owned == false) draw_scope.hdromeo_FD = false
		
		// Not the weapon we're looking for
		if(vm.GetModelName() != hdromeo_MDL)
			continue
		
		// initial deploy check
		ply.ValidateScriptScope()
		local draw_scope = ply.GetScriptScope()
		if( !draw_scope.rawin("hdromeo_FD") )
		{
			draw_scope.hdromeo_FD <- false
			draw_scope.hdromeo_owned <- false
		}
		
		if (draw_scope.hdromeo_FD == false)
		{
			vm.__KeyValueFromInt("sequence", hdromeo_sequence)
			draw_scope.hdromeo_FD = true
		}
	}
}
...
if (deployTimerEnt == null)
{
	...
	EntFireByHandle(MIGI_deploy_timer, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:hdromeo_deployCheck():0:-1", 0, null, null)
	EntFireByHandle(MIGI_deploy_timer, "Enable", "", 0.1, null, null)
}
else
{
	EntFireByHandle(deployTimerEnt, "AddOutput", "OnTimer "+self.GetName()+":RunScriptCode:hdromeo_deployCheck():0:-1", 0, null, null)
